...
@test "MyPlace Test Set Active 0 z01 - close z01" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/basicPassingSystemData.txt"
   run ../MyPlace.sh Set Fanv2 Active 0 z01 TEST_ON 127.0.0.1
...
@test "MyPlace Test Set Active 0 z02 - close z02 immediately after closing z01" {
   run ../MyPlace.sh Set Fanv2 Active 0 z02 TEST_ON 127.0.0.1
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Using IP: 127.0.0.1"
   assert_equal "${lines[1]}" "Getting myAirData.txt from cached file"
   ...
   # read from zoneOpen.txt
   assert_equal "${lines[8]}" "Setting url: http://127.0.0.1:2025/setAircon?json={ac1:{zones:{z02:{state:close}}}}"
   assert_equal "${lines[9]}" "Try 0"
   assert_equal "${lines[10]}" "Setting json: .aircons.ac1.zones.z02.state=\"close\""
   # No more lines than expected
