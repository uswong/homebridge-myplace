...
@test "MyPlace Test Get TargetTemperature (ac1 myZone=0)" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/basicPassingSystemData.txt"
   run ../MyPlace.sh Get Blah TargetTemperature 127.0.0.1 TEST_ON
...
@test "MyPlace Test Get TargetTemperature (ac2 myZone=7)" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/myPlaceFull.txt"
   run ../MyPlace.sh Get Blah TargetTemperature 127.0.0.1 ac2 TEST_ON
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Try 0"
