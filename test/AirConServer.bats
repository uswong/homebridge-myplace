...
@test "StartServer Test Load/Set/Read new data" {
   ...
   assert_equal "$status" 0
   assert_equal "${#lines[@]}" 0
   # MyPlace.sh TargetTemperature 9
...
@test "StartServer Test MyPlace.sh fails appropriately when no data loaded" {
   beforeEach
   # Issue the reInit
...
@test "StartServer Test Load/Set/Read new data using MyPlace.sh" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   ...
   run ../MyPlace.sh Get Blah TargetHeatingCoolingState 127.0.0.1 TEST_ON
   assert_equal "$status" 0
   # MyPlace.sh does a get first
   assert_equal "${lines[0]}" "Try 0"
   ...
   run ../MyPlace.sh Set Blah TargetHeatingCoolingState 0 127.0.0.1 TEST_ON
   assert_equal "$status" 0
