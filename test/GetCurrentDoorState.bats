...
@test "MyPlace Test Get CurrentDoorState thiID:6801801" {
   beforeEach
   # Issue the reInit
...
@test "MyPlace Test Get CurrentDoorState thiID:6801801 flip enabled" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/myPlace.txt"
   run ../MyPlace.sh Get "Garage" CurrentDoorState thiID:6801801 flip 127.0.0.1 TEST_ON
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Try 0"
   assert_equal "${lines[1]}" "Parsing for jqPath: .aircons.ac1.info"
   assert_equal "${lines[2]}" "Parsing for jqPath: .myThings.things.\"6801801\".value"
