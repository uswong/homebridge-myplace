...
@test "MyPlace Test Set RotationSpeed 83 - fanSpeed" {
   beforeEach
   # Issue the reInit
   ...
   run ../MyPlace.sh Set Blah RotationSpeed 83 TEST_ON 127.0.0.1 fanSpeed
...
@test "MyPlace Test Set RotationSpeed 78 - z01 ac1 (rssi=0 and zoneSpecified=true)" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   ...
   # Bats "run" gobbles up all the stdout. Remove for debugging
   run ../MyPlace.sh Set Blah RotationSpeed 78 TEST_ON z01 127.0.0.1
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Using IP: 127.0.0.1"
   assert_equal "${lines[1]}" "Try 0"
   assert_equal "${lines[2]}" "Parsing for jqPath: .aircons.ac1.info"
...
@test "MyPlace Test Set RotationSpeed 78 - z01 ac2 (rssi!=0 and zoneSpecified=true)" {
   beforeEach
   # Issue the reInit
   curl -s -g "http://localhost:$PORT/reInit"
   # Do the load
   curl -s -g "http://localhost:$PORT?load=testData/myPlaceFull.txt"
   # Bats "run" gobbles up all the stdout. Remove for debugging
   run ../MyPlace.sh Set Blah RotationSpeed 78 TEST_ON z01 127.0.0.1 ac2
   assert_equal "$status" 0
   assert_equal "${lines[0]}" "Using IP: 127.0.0.1"
   assert_equal "${lines[1]}" "Try 0"
