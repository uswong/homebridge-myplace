...
async function main({
  ...
  for (let n = 1; n <= noOfTablets; n++) {
    ...
    try {
      ...
      console.error(`ERROR: AdvantageAir system is inaccessible - make sure it is power ON and its IP address ${IPA} is correct.`);
      process.exit(1);
    }
    ...
    if (!sysName) {
      console.error("ERROR: failed to get system info from your AdvantageAir system!");
      process.exit(1);
    }
  ...
  console.error("DONE! createMyPlaceConfig completed successfully!");
