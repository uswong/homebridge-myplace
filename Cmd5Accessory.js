...
class Cmd5Accessory
{
      ...
      if ( this.loggingService )
      {
         ...
         switch ( this.eve )
         {
            ...
            case constants.FAKEGATO_TYPE_THERMO:
            {
               ...
               if ( settings.cmd5Dbg ) this.log.debug( `Logging ${ constants.CURRENTTEMP }: ${ firstParmValue } ${ constants.SETTEMP }:${ secondParmValue } ${constants.VALVEPOSITION }:${ thirdParmValue } ` );
            ...
            case constants.FAKEGATO_TYPE_AQUA:
            {
               ...
               if ( settings.cmd5Dbg ) this.log.debug( `Logging ${ constants.STATUS }: ${ firstParmValue } ${ constants.WATERAMOUNT }: ${ secondParmValue }` );
   ...
   setupAccessoryFakeGatoService( fakegatoConfig )
   {
      ...
      for ( let key in fakegatoConfig )
      {
         ...
         let rcDirective = isCmd5Directive( key, false );
         if ( rcDirective == null )
         {
            rcDirective = isCmd5Directive( key, true );
            if ( rcDirective != null )
            ...
            case constants.EVE:
               ...
               switch( value )
               {
                   ...
                   case constants.FAKEGATO_TYPE_AQUA:
                      break;
                   default:
                      throw new Error( `Invalid fakegato eve type: "${ value }". It must be one of ( ${ constants.FAKEGATO_TYPE_ENERGY }, ${ constants.FAKEGATO_TYPE_ROOM }, ${ constants.FAKEGATO_TYPE_WEATHER }, ${ constants.FAKEGATO_TYPE_DOOR }, ${ constants.FAKEGATO_TYPE_MOTION }, ${ constants.FAKEGATO_TYPE_THERMO }, ${ constants.FAKEGATO_TYPE_AQUA } ). Check the Cmd5 README at: "https://github.com/simont77/fakegato-history".` );
                }
                break;
            ...
            case constants.VALVEPOSITION:
            {
               if ( value != "0" )
               {
                  ...
                  if ( rcDirective.accTypeEnumIndex == null )
                  {
                    ...
                    this.log.warn( `The config.json FakeGato characteristic: ${ value } is Capitalized it should be: ${ rcDirective.type }.  In the near future this will be an Error so that Cmd5 can use homebridge-ui.\nTo remove this Warning, Please fix your config.json.` );
                  }
      ...
      if ( this.storage != undefined )
      {
         ...
         {
            this.log.warn( chalk.yellow( "WARNING" ) + `: Cmd5 Unknown accessory config.storage:{ this.storage } Expected:${ constants.FS } or ${ constants.GOOGLEDRIVE } for: ${ this.displayName }` );
         }
      }
   ...
   parseKeyForCharacteristics( key, value, parseConfigShouldUseCharacteristicValues )
   {
      ...
      if ( Object.keys( CMD5_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].validValues ).length > 0 )
      {
         // Even if outputConsts is not set, just in case, transpose it anyway.
         value = transposeConstantToValidValue( CMD5_ACC_TYPE_ENUM.properties, accTypeEnumIndex, value ) ;
      }


      // Return the appropriate type, by seeing what it is defined as in Homebridge,
      let properValue = CMD5_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].stringConversionFunction( value );
      if ( properValue == undefined )
      {
         // If the value is not convertable, just return it.
         this.log.warn( `parseKeyForCharacterisitcs: ${ this.displayName } ` + chalk.red( `Cannot convert value: ${ value } to ${ CMD5_ACC_TYPE_ENUM.properties[ accTypeEnumIndex ].props.format } for ${ characteristicString }`  ) );
      ...
      this.cmd5Storage.setStoredValueForIndex( accTypeEnumIndex, properValue );
   }
   ...
   processRequires( requiresArray )
   {
      ...
      for ( let argIndex = 0; argIndex < requiresArray.length; argIndex++ )
      {
         ...
         if ( settings.cmd5Dbg ) this.log.debug( `Requiring ${ required }` );
   ...
   accessoryTypeConfigToCmd5Accessories( config, parentInfo )
   {
      if ( ! config )
         return undefined;
      ...
      {
         let accessories = config.map( ( accessoryConfig ) => { return new Cmd5Accessory( that.log, accessoryConfig, this.api, this.STORED_DATA_ARRAY, this ) } );

         // Put the accessories into their correct collection array.
         parentInfo.createdCmd5Accessories.push( ...accessories );

         return accessories;
      }

      let accessory = new Cmd5Accessory( that.log, config, this.api, this.STORED_DATA_ARRAY, this );

      // Put the accessory into its correct collection array.
      parentInfo.createdCmd5Accessories.push( accessory );
   ...
   parseConfig( config, parseConfigShouldUseCharacteristicValues  )
   {
      ...
      for ( let key in config )
      {
         ...
         if ( rcDirective == null )
         {
            ...
            if ( rcDirective != null )
            {
               this.log.warn( `The config.json Cmd5 Directive: ${ key } is Capitalized.  It should be: ${ rcDirective.key }. In the near future this will be an error for homebridge-ui integration.\nTo remove this Warning, Please fix your config.json.` );
               // create the proper lower case value
         ...
         switch ( key )
         {
            case constants.TYPE:
            {
               ...
               if ( rcValue.devEnumIndex == null )
               {
                  ...
                  // warn now
                  this.log.warn( `The config.json Cmd5 device type: ${ value } is lowerCase.  It should be: ${ rcValue.deviceName }. In the near future this will be an error for homebridge-UI integration.\nTo remove this Warning, Please fix your config.json.` );
           ...
           case constants.CATEGORY:
            ...
            case constants.CONSTANTS:
               this.log.warn( `Warning: ${ key } will soon been deprecated at the Accessory level. Please move it to where "Platform: "Cmd5" is located in your config.json.` );
            ...
            case constants.LINKEDTYPES:
               if ( settings.cmd5Dbg ) this.log.debug(  `parseConfig. Found linked Accessories` );
               this.linkedAccessoriesConfig = value;

               break;
            case constants.ACCESSORIES:
               if ( settings.cmd5Dbg ) this.log.debug( `parseConfig. Found Accessories` );
            ...
            case constants.ALLOWTLV8:
               this.log.warn( `Warning: ${ key } will soon been deprecated at the Accessory level. Please move it to where "Platform: "Cmd5" is located.` );
               this.log.warn( `This message will disappear when you have done so.` );
      ...
      if ( cmd5Mode != null && this.polling )
      {
         if ( cmd5Mode == "Demo" )
            throw new Error("Demo mode is achieved when there are no polling entries in your config.json");

         this.log.warn( `Cmd5 has been simplified and optimized as per: https://git.io/JtMGR.` );
         this.log.warn( `To remove this message, just remove "Cmd5_Mode" from your config.json` );
      }
      ...
      } else
      {
         ...
         if ( this.level != 1)
            this.log.info( chalk.blue( `Cmd5 is running in Demo Mode for ${ this.displayName }` ) );
      }
      ...
      if ( this.typeIndex == CMD5_DEVICE_TYPE_ENUM.Television )
      {
          if ( this.CMD5 == constants.PLATFORM &&  ( ! this.publishExternally || ! this.category ) ||
               this.CMD5 == constants.STANDALONE )
          {
          ...
          if ( this.CMD5 == constants.PLATFORM && ! this.publishExternally && ( numberOfTVsPerBridge += 1 ) > 1 )
          {
      ...
      switch( pollingType )
      {
         case null:
         case undefined:
            if ( settings.cmd5Dbg ) this.log.debug( `No polling configured.` );
            return;
         case Boolean:
            if ( settings.cmd5Dbg ) this.log.debug( `Polling config is Default Polling. Nothing to check for unset polling characteristics` );
            return;
         case String:
      ...
      for ( let accTypeEnumIndex = 0; accTypeEnumIndex < CMD5_ACC_TYPE_ENUM.EOL; accTypeEnumIndex ++ )
      {
         ...
         if ( storedValue != null )
         {
            ...
            let lcCharacteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToLC( accTypeEnumIndex );
            let ucCharacteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );
   ...
   getDevicesRelatedTargetAccTypeEnumIndex( accCurrentEnumIndex )
   {
      ...
      let requiredCharacteristicsArray = CMD5_DEVICE_TYPE_ENUM.properties[ this.typeIndex ].requiredCharacteristics;
   ...
   determineCharacteristicsToPollForAccessory( accessory  )
   {
      ...
      {
         if ( settings.cmd5Dbg ) this.log.debug( `Characteristic polling for: ${ accessory.displayName }` );
         accessory.polling.forEach( ( jsonPollingConfig ) =>
         {
               ...
               switch ( key )
               {
                  ...
                  case constants.CHARACTERISTIC:
                  {
                     ...
                     if ( rcDirective.accTypeEnumIndex == null )
                     {
                        ...
                        this.log.warn( `1. The config.json Polling characteristic: ${ value } is Capitalized it should be: ${ rcDirective.type }.  In the near future this will be an Error so that Cmd5 can use homebridge-ui.\nTo remove this Warning, Please fix your config.json.` );
                     }
                     accTypeEnumIndex = rcDirective.accTypeEnumIndex;
            ...
            let characteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToUC( accTypeEnumIndex );

            if ( settings.cmd5Dbg ) this.log.debug( `Setting up accessory: ${ accessory.displayName } for polling of: ${ characteristicString } timeout: ${ timeout } interval: ${ interval } queueName: "${ this.queue.queueName }"` );
      ...
      } else
      {
         ...
         CMD5_DEVICE_TYPE_ENUM.properties[ accessory.typeIndex ].defaultPollingCharacteristics.forEach( defaultPollingAccTypeEnumIndex =>
         {

            // This has to be UC as it gets passed to the getValue cmd string
            let characteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToUC( defaultPollingAccTypeEnumIndex );
            ...
            if ( accessory.polling == true )
            {
               ...
               if ( settings.cmd5Dbg ) this.log.debug( `Adding ${ record.accessory.displayName } ${ CMD5_ACC_TYPE_ENUM.properties[ record.accTypeEnumIndex ].type }  record.timeout: ${ record.timeout } record.interval: ${ record.interval }  to Polled Queue ${ record.queueName }` );
      ...
      for( let accTypeEnumIndex in this.listOfPollingCharacteristics )
      {
         ...
         if ( relatedTargetAccTypeEnumIndex != null )
         {
               ...
               )
            {
               ...
               let characteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToLC( accTypeEnumIndex );
               let relatedCharacteristicString = CMD5_ACC_TYPE_ENUM.accEnumIndexToLC( relatedTargetAccTypeEnumIndex );
...
function checkAccessoryForDuplicateUUID( accessory, uuid )
{
   ...
   if ( settings.cmd5Dbg ) accessory.log.debug( `Checking ${ accessory.name } for Duplicate UUID: ${ accessory.uuid }` );

   for ( let existingAccessory in accessory.createdCmd5Accessories )
   {
   ...
   if ( settings.cmd5Dbg ) accessory.log.debug( `No Duplicate UUID's for this Accessory - ` + chalk.green( `OK` ) + `. Using: ${ accessory.uuid }` );
}

exports.Cmd5Accessory = Cmd5Accessory;
